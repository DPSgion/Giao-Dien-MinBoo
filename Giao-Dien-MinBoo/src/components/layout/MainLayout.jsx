...
export default function MainLayout({ children }) {
  ...
  // Ref để theo dõi userId hiện tại, tránh fetch lại không cần thiết
  const currentUserIdRef = useRef(null);
  const isFetchingRef = useRef(false);

  // Fetch unread count - được wrap trong useCallback để ổn định
  const fetchUnreadCount = useCallback(async () => {
    // Tránh gọi chồng chéo
    if (isFetchingRef.current) return;
    isFetchingRef.current = true;

    try {
      // Gọi đồng thời hai API để tăng tốc
      const [notifRes, friendRes] = await Promise.allSettled([
      ...
      if (notifRes.status === 'fulfilled') {
        ...
        newNotif = data.notifications_unread || 0;
        newMsg = data.messages_unread || 0;
      }

      // Xử lý friend requests
      let newFriendReq = 0;
      if (friendRes.status === 'fulfilled') {
        const res = friendRes.value;
        const received = Array.isArray(res) ? res : res?.data || [];
        newFriendReq = received.length;
      }

      // Chỉ cập nhật state một lần duy nhất, giảm số lần render
      setUnread((prev) => {
        if (
          prev.notifications === newNotif &&
          prev.messages === newMsg &&
          prev.friendRequests === newFriendReq
        ) {
      ...
      console.error('Lỗi fetchUnreadCount:', error);
    } finally {
      isFetchingRef.current = false;
    }
  }, []);

  // Effect chính: kết nối WebSocket và đăng ký listener
  useEffect(() => {
    ...
    // Kết nối WebSocket
    websocketService.connect(user.user_id, token);
    ...
    // Cleanup: disconnect khi unmount hoặc user thay đổi
    return () => {
      websocketService.disconnect();
      // Lưu ý: không remove listener ở đây vì websocketService nên tự quản lý
    };
  }, [user?.user_id, fetchUnreadCount]);

  // Effect riêng để fetch unread count khi user thay đổi (chỉ chạy khi user_id khác)
  useEffect(() => {
    // Nếu chưa có user hoặc userId không thay đổi so với lần trước thì không fetch
    if (!user?.user_id) return;
    if (currentUserIdRef.current === user.user_id) return;

    currentUserIdRef.current = user.user_id;
    fetchUnreadCount();
  }, [user?.user_id, fetchUnreadCount]);

  // Listener cho sự kiện từ UI (khi gửi/hủy kết bạn)
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen bg-white">
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-full border-r border-gray-200 bg-white z-40 flex flex-col transition-all duration-300 ${
          collapsed || showSearch ? 'w-20' : 'w-64'
        }`}
      >
        {/* Logo */}
        ...
        <nav className="flex-1 px-3 space-y-1">
          ...
          <Link
            to="/create"
            className={`flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-100 transition-colors ${
              isActive('/create') ? 'font-bold' : 'font-normal'
            }`}
          >
        ...
        <div className="px-3 pb-5 space-y-1">
          ...
          <Link
            to={`/profile/${user?.user_id || user?.id || 'me'}`}
            className="flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-100 transition-colors"
          >
            {(user?.url_avt || user?.avatar) &&
            !(user?.url_avt || user?.avatar || '').includes(
              'default-avatar-url'
            ) ? (
              <img
      ...
      {/* Main content */}
      <main
        className={`flex-1 min-h-screen transition-all duration-300 ${collapsed || showSearch ? 'ml-20' : 'ml-64'}`}
      >
        {children}
      </main>
    </div>
