...
export default function Friends() {
  ...
  const fetchFriends = async () => {
    ...
    try {
      ...
      const mapped = rawFriends.map((f) => {
        const myId = user?.user_id || user?.id;
        const isRequester =
          f.requesterId === myId || String(f.requesterId) === String(myId);
        return {
  ...
  const fetchPending = async () => {
    ...
    try {
      ...
      const receivedRaw = Array.isArray(pendingRes)
        ? pendingRes
        : pendingRes?.data || [];
    ...
    try {
      ...
      setPending((prev) => ({
        ...
        received: prev.received.filter(
          (r) => r.friend_request_id !== friendRequestId
        ),
      }));
  ...
  const handleDecline = async (friendRequestId, isSent = false) => {
    try {
      ...
      console.log('handleDecline Error:', err);
    }
  };
  ...
  const handleUnfriend = async (userId) => {
    ...
    try {
      ...
      console.log('handleUnfriend ở friend gặp Error:', err);
    }
  };
  ...
  const handleMessage = async (userId) => {
    try {
      ...
      if (convId) {
        ...
        console.warn('Không lấy được conversation_id từ response:', res);
        navigate(`/messages`);
      }
    } catch (err) {
      console.error('handleMessage Error:', err);
      alert(
        'Lỗi khi tạo/lấy cuộc trò chuyện: ' +
                ...
                >
                  <Link to={`/profile/${friend.user_id}`} className="relative">
                    <img
                      src={
                        friend.url_avt ||
                        `https://ui-avatars.com/api/?name=${friend.name}&background=random&size=100`
                      }
                ...
                {pending.received.map((req) => (
                  ...
                  >
                    <Link to={`/profile/${req.from_user?.user_id}`}>
                      <img
                        src={
                          req.from_user?.url_avt ||
                          `https://ui-avatars.com/api/?name=${req.from_user?.name}&background=random`
                        }
                    ...
                    <div className="flex-1 min-w-0">
                      <Link
                        to={`/profile/${req.from_user?.user_id}`}
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        onClick={() =>
                          handleAccept(req.friend_request_id, req.from_user)
                        }
                  ...
                  >
                    <Link to={`/profile/${req.to_user?.user_id}`}>
                      <img
                        src={
                          req.to_user?.url_avt ||
                          `https://ui-avatars.com/api/?name=${req.to_user?.name}&background=random`
                        }
                        className="w-12 h-12 rounded-full object-cover"
                    ...
                    <div className="flex-1 min-w-0">
                      <Link
                        to={`/profile/${req.to_user?.user_id}`}
                        className="font-semibold text-sm hover:underline block"
                      >
                        {req.to_user?.name}
                      </Link>
                      <p className="text-xs text-gray-400 mt-0.5">
                        Đang chờ phản hồi • {formatTime(req.created_at)}
                      </p>
                    </div>
                    <button
