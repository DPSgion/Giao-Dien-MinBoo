...
export const friendService = {
  ...
  sendRequest: (friendId, message) =>
    axiosClient.post(`/friends/request/${friendId}`, {
      message: message || '',
    }),
  ...
  unfriend: (friendId) => axiosClient.delete(`/friends/unfriend/${friendId}`),
};
...
export const messageService = {
  // [API 9.1] GET /conversations - Lấy danh sách cuộc hội thoại
  getConversations: () => axiosClient.get('/conversations'),

  // [API 9.2] GET /conversations/{conversationId}/messages - Lấy tin nhắn
  getMessages: (conversationId, params) =>
    axiosClient.get(`/conversations/${conversationId}/messages`, { params }),

  // [API 9.3] POST /conversations/{conversationId}/messages - Gửi tin nhắn
  sendMessage: (conversationId, formData) =>
    axiosClient.post(`/conversations/${conversationId}/messages`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    }),

  // [API 9.4] POST /conversations/{conversationId}/seen - Đánh dấu đã xem
  markSeen: (conversationId) =>
    axiosClient.post(`/conversations/${conversationId}/seen`),

  // [API 9.5] POST /conversations - Tạo cuộc trò chuyện mới với 1 người
