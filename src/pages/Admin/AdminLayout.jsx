...
const navItems = [
  ...
  },
  {
    ...
    hasBadge: true,
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
  },
  {
    path: "/admin/tags",
    label: "Quản lý Tags",
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>,
  },
];
    ...
    <div className="flex min-h-screen" style={{ background: "#0a0a0f" }}>
      ...
      <aside className={`fixed left-0 top-0 h-full z-40 flex flex-col transition-all duration-200
        ...
        <div className={`flex items-center gap-3 px-4 py-4 flex-shrink-0`}
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-white font-black text-sm leading-none">MinBoo</p>
              <p className="text-xs mt-0.5" style={{ color: "#7c3aed" }}>Admin Panel</p>
            </div>
          )}
        ...
        <div className="px-2 pb-3 space-y-0.5 flex-shrink-0">
          ...
          {/* Admin info */}
          <div className="flex items-center gap-3 px-3 py-2 mt-1 rounded-xl"
            style={{ background: "#1a1a24" }}>
            <img
              src={user?.url_avt || `https://ui-avatars.com/api/?name=${user?.name || "A"}&background=7c3aed&color=fff`}
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="text-xs" style={{ color: "#7c3aed" }}>Administrator</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ── MAIN ── */}
      <main className={`flex-1 transition-all duration-200 ${collapsed ? "ml-16" : "ml-60"} flex flex-col min-h-screen`}>
        {/* Topbar */}
        <div className="sticky top-0 z-30 flex items-center justify-between px-6 py-3"
          style={{ background: "rgba(17,17,24,0.9)", backdropFilter: "blur(12px)", borderBottom: "1px solid #1f1f2e" }}>
          <div>
            <h1 className="text-white font-black text-base">{currentPage.label}</h1>
            <p className="text-xs" style={{ color: "#4b5563" }}>MinBoo Management System</p>
          </div>
