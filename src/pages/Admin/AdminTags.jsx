    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: "#6b7280" }}>
          Tổng <span style={{ color: "#10b981" }} className="font-semibold">{tags.length}</span> tags
        </p>
      </div>
      ...
      {/* Tags grid */}
      <div className="rounded-2xl p-5" style={{ background: "#111118", border: "1px solid #1f1f2e" }}>
        <h3 className="text-white font-bold text-sm mb-4">Danh sách tags hiện có</h3>
        {loading ? (
          <div className="flex flex-wrap gap-3">
            {Array(10).fill(0).map((_, i) => (
              <div key={i} className="h-8 w-20 rounded-full animate-pulse" style={{ background: "#1f1f2e" }} />
            ))}
          </div>
        ) : tags.length === 0 ? (
          <p className="text-sm text-center py-8" style={{ color: "#4b5563" }}>Chưa có tag nào</p>
        ) : (
            ...
            {tags.map((tag, i) => {
              ...
              return (
                <div key={tag.tag_id}
                  ...
                  <span className="text-xs opacity-50 font-normal">ID:{tag.tag_id}</span>
                </div>
              );
            })}
          </div>
