...
export default function AdminUsers() {
  ...
  const card = (bg, border) => ({ background: bg, border: `1px solid ${border}` });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#6b7280" }}>
            Tổng <span style={{ color: "#7c3aed" }} className="font-semibold">{pagination.total?.toLocaleString()}</span> tài khoản
          </p>
        </div>
        {/* Filter tabs */}
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#1a1a24", border: "1px solid #1f1f2e" }}>
          {[["all","Tất cả"], ["true","Hoạt động"], ["false","Bị khóa"]].map(([v, l]) => (
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="relative flex-1">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
            className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: "#4b5563" }}>
        ...
        <button type="submit" className="text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors"
      ...
      {/* Table */}
      <div className="rounded-2xl overflow-hidden" style={card("#111118", "#1f1f2e")}>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid #1f1f2e" }}>
                {["Người dùng", "Email", "Ngày tham gia", "Role", "Trạng thái", "Thao tác"].map((h) => (
                ...
                <tr key={i} style={{ borderBottom: "1px solid #1a1a24" }}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3 animate-pulse">
                      <div className="w-9 h-9 rounded-full" style={{ background: "#1f1f2e" }} />
                      <div className="space-y-1.5">
                        <div className="h-3 rounded w-28" style={{ background: "#1f1f2e" }} />
                        <div className="h-2 rounded w-20" style={{ background: "#161620" }} />
                      </div>
                    </div>
                  </td>
                  {[1,2,3,4,5].map((j) => (
                    <td key={j} className="px-5 py-3">
                      <div className="h-3 rounded w-20" style={{ background: "#1f1f2e" }} />
                    </td>
                  ))}
                </tr>
              )) : users.length === 0 ? (
                ...
                <tr key={u.user_id} className="transition-colors"
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <img
                        src={u.url_avt || `https://ui-avatars.com/api/?name=${u.name}&background=7c3aed&color=fff&size=64`}
                        className="w-9 h-9 rounded-full object-cover flex-shrink-0"
                        alt={u.name}
                      />
                      <div>
                        <p className="text-sm font-semibold text-white">{u.name}</p>
                        <p className="text-xs" style={{ color: "#4b5563" }}>@{u.username}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-sm" style={{ color: "#6b7280" }}>{u.email || "—"}</td>
                  ...
                  <td className="px-5 py-3">
                    <span className="text-xs px-2.5 py-1 rounded-full font-medium"
                      style={u.role === 1
                        ...
                        : { background: "#1f1f2e", color: "#6b7280", border: "1px solid #2a2a38" }}>
                      {u.role === 1 ? "Admin" : "User"}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3">
                    <button
                      ...
                      style={u.is_active
                        ? { background: "rgba(239,68,68,0.15)", color: "#f87171", border: "1px solid rgba(239,68,68,0.3)" }
                        : { background: "rgba(16,185,129,0.15)", color: "#34d399", border: "1px solid rgba(16,185,129,0.3)" }}>
                      {togglingId === u.user_id ? "..." : u.is_active ? "🔒 Khóa" : "🔓 Mở khóa"}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        {/* Pagination */}
        {pagination.total > 15 && (
          <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: "1px solid #1f1f2e" }}>
            ...
            <div className="flex gap-2">
              {[["← Trước", -1], ["Sau →", 1]].map(([label, dir]) => (
                <button key={label}
                  ...
                  onClick={() => { const p = page + dir; setPage(p); fetchUsers(search, filterActive, p); }}
